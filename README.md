...
## 1M-step benchmark (in progress)

*Results pending. Run `bash scripts/reproduce_hopper_v4_1m.sh` to populate.*

![Eval return: 1M benchmark](reports/figures/1m/compare_eval_return.png)

*Plot will be generated by the reproduce script into `reports/figures/1m/`.*
...
Identical hyperparameters to the 20k run; only `--total_timesteps` and `--eval_every` change. At 1M steps Hopper-v4 is within reach of meaningful locomotion performance for state-based PPO (typical benchmark range 1000–3000 return). Pixel-based modes are expected to show a narrowing gap relative to the 20k results, though the extent of this depends on whether the CNN encoder stabilises during training.
...
- **Frame stacking (Phase 6 — complete).** `--frame_stack 4` is implemented and benchmarked. Stack=4 yields 65.6 ± 14.2 at 20k steps vs 50.1 ± 3.1 for no-stack pixels — a partial improvement with higher variance. Longer training horizons would be needed to determine whether the improvement is sustained.
